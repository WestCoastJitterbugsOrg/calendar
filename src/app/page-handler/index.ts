 ...
 */
const PageHandlerFactory: Factory<PageHandlerCreator> =
    deps => (events: WcjEvent[]) => {
        const $ = deps.$;
        const eventGroupList = deps.eventGroupList(events);
        const calendar = deps.fullerCalendar($('#calendar').get(0));
        const uniqueEvents = deps.getUniqueEvents(events)
        // Create all checkboxes out of the unique events
        for (const event of uniqueEvents) {
            const checkboxEl = $(`<input type="checkbox" class="courseCheckbox" id="course-${event.id}">`)
                .on("change", function () {
                    ...
                    deps.setEvents(calendar, eventGroupList.getSelected());
                });
            const labelEl = $(`<label for="course-${event.id}" class="courseCheckboxLabel">${event.title}</label>`);
...
export default PageHandlerFactory