...
app.get(
  "/api/wcj-courses",
  handleDanSeData(
    `https://dans.se/xml/?type=courses&org=wcj&pw=${pw}&regStatus=0&dateInterval=future`
  )
);
// #endregion
...
app.get("/api/gcal", async function (req, res) {
  try {
    const events = await google
      ...
      .events.list({
        calendarId:
          "wcj.se_57n2cj034c49cirl0rl20t3io4@group.calendar.google.com",
        alwaysIncludeEmail: false,
        timeMin:
          req.query.from?.toString() || new Date(2019, 8, 1).toISOString(),
        timeMax:
          req.query.to?.toString() || new Date(2019, 11, 1).toISOString(),
      });
